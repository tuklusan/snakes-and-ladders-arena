...
function makeAudio(resolveAfterPlay) {
  const audio = new window.HTMLAudioElement();
...
function testAutoplayBlocked() {
  ...
  let buttonInDom = document.getElementById('start-arena-btn') !== null;
  console.log(`Button in DOM after probe: ${buttonInDom} (expected: true)`);
...
// Run tests
let passed = true;
passed = testAutoplayAllowed() && passed;
passed = testAutoplayBlocked() && passed;
