...
class GameView {
    ...
    assetError(e) {
        ...
        this.assetLoaded(); // still count as loaded to avoid hanging, but we could want to retry
    }
    ...
    handleRollClick() {
        ...
        // Re-enable after a short delay (or when the turn is over)
        setTimeout(() => {
            if (!this.model.isGameOver()) {
                this.rollButton.disabled = false;
            }
    ...
    playAudio(event) {
        ...
        if (audio) {
            audio.currentTime = 0;
            audio.play().catch(e => console.warn(`Audio play failed for ${e}: ${event}`));
        }
    }
    ...
    onStateChange() {
        ...
        // Re-enable roll button if not game over
        if (!this.model.isGameOver()) {
            this.rollButton.disabled = false;
        }
    }
